...
const correctType = (type) => {
    ...
    if (['object', 'file'].indexOf(type) !== -1) return 'any';
    return type;
};
...
module.exports = function generate(ctx) {
    ...
    function buildObservableType(model, method) {
        let type = 'any';
        const methodReturn = Array.isArray(method.returns) && method.returns.length > 0 ? method.returns[0] : method.returns;
        const hasDefinedReturn = typeof methodReturn === 'object' && !!methodReturn.type;
        const returnTypeIsDefinedArray = Array.isArray(methodReturn.type);
        const returnTypeIsSimpleArray = String(methodReturn.type).toLowerCase() === 'array';
        const returnsArray = hasDefinedReturn && (returnTypeIsSimpleArray || returnTypeIsDefinedArray);
        if (hasDefinedReturn) {

            if (returnTypeIsDefinedArray && methodReturn.type.length > 0) {
                type = `${correctType(methodReturn.type[0])}[]`;
            } else if (returnTypeIsSimpleArray) {
                type = `any[]`;
            } else {
                type = correctType(methodReturn.type);
            }
        } else {
            if (method.name.match(/(^createMany$|^find)/g) || returnsArray) {
                type = `${model.name}[]`;
            }
    ...
    /**
     * @method buildReturnType
     * @description
     * Define return type
     */
    function buildReturnType(model, method) {
        const methodReturn = Array.isArray(method.returns) && method.returns.length > 0 ? method.returns[0] : method.returns;
        const hasDefinedReturn = typeof methodReturn === 'object' && !!methodReturn.type;
        if (hasDefinedReturn) return correctType(typeof methodReturn.type === 'string' ? methodReturn.type : methodReturn.type[0]);
        return model.name;
    }

    function hasDefinedReturnType(method) {
        const methodReturn = Array.isArray(method.returns) && method.returns.length > 0 ? method.returns[0] : method.returns;
        return typeof methodReturn === 'object' && !!methodReturn.type;
    }
    ...
    function buildServiceImports(model, loadAccessToken, isIo) {
        ...
        model.methods.forEach(method => {
            ...
            const hasDefinedReturn = typeof methodReturn === 'object' && !!methodReturn.type;
            if (hasDefinedReturn) {
                const returnModelName = typeof methodReturn.type === 'string' ? methodReturn.type : methodReturn.type[0];
                if (!loaded[returnModelName] && ['object', 'any', 'array', 'file', 'number', 'integer', 'boolean', 'float', 'string', 'text'].indexOf(returnModelName) === -1) {
                    loaded[returnModelName] = true;
                    imports.push({
                        module: returnModelName,
                        from: `../../models/${returnModelName}`,
                    });
                }
            }
        });
    ...
    function buildBaseServiceImports(isIo) {
        let imports = [
            {module: 'Injectable, Inject, Optional', from: '@angular/core'},
            {
                module: 'HttpClient, HttpHeaders, HttpRequest, HttpParams, HttpResponse, HttpParameterCodec',
